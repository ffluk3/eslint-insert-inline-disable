...
function runEslint() {
  ...
  const report = linter.executeOnFiles(
    globSync(path.join(process.cwd(), args[0]))
  );

  const relevantResults = report.results.filter((res) =>
    res.messages.some((message) => message.severity === 2)
  );
  ...
  relevantResults.forEach((result) => {
    ...
    const relevantMessages = result.messages.filter((message) =>
      options.rules.includes(message.ruleId)
    );
    ...
    relevantMessages.forEach((ruleViolation) => {
      ...
      resultMap[moduleName] = resultMap[moduleName]
        ? resultMap[moduleName] + 1
        : 1;
    });
